...
| **Phase 3** — Ecosystem | **4.x** (and a future major release for breaking removals) | ML dtype support, engine wrappers for `zarrs` and TensorStore, Arrow integration investigation. The eager `__getitem__` path is removed in a future major release after its deprecation window completes (per [lazy-indexing.md § Migration](./proposals/lazy-indexing.md#migration)). |

Phases are not gated on calendar dates; the work ships when it's ready. The release column above describes which *kind* of release each phase corresponds to (a major 4.0 vs incremental 4.x minor releases vs a future major release for breaking removals), not when.
...
5. **Lazy indexing — the composable coordinate-mapping algebra** — the foundational `IndexTransform` data structure that lazy `__getitem__` is built on. **Ahead of the rest of Phase 0**: the library is already in flight at [zarr#3906](https://github.com/zarr-developers/zarr-python/pull/3906). The 4.0 migration adds an opt-in `array.lazy[...]` accessor to the existing `Array` class; a later 4.x release flips the default of bare `array[...]`; the eager path is removed in a future major release after the deprecation window. No new array type is ever introduced — the codebase carries one `Array` whose `__getitem__` semantics evolve across releases. → [proposals/lazy-indexing.md](./proposals/lazy-indexing.md)
