...
TensorStore in particular already owns a Zarr-shaped segment that Zarr-Python does not currently serve: **large-scale distributed checkpointing for JAX / LLM training**. Google's [Orbax](https://github.com/google/orbax) (the de facto JAX checkpointing library) backends onto TensorStore; so do Apple's [AXLearn](https://github.com/apple/axlearn), NVIDIA's [Megatron-LM](https://github.com/NVIDIA/Megatron-LM) and [NeMo](https://github.com/NVIDIA/NeMo) (`dist_checkpointing/strategies/tensorstore.py`), Stanford/Marin's [Levanter](https://github.com/marin-community/levanter), [MLCommons training benchmarks](https://github.com/mlcommons/training), and the JAX team's own example repos. The engine-wrapping work is what lets those workloads use Zarr-Python's API surface (and the rest of Zarr-Python's ecosystem) while delegating IO to TensorStore — closing a documented adoption gap rather than chasing a hypothetical one.
...
- **Microsoft** — Megatron-LM distributed checkpointing strategies include a Zarr backend; [Planetary Computer](https://planetarycomputer.microsoft.com/) hosts Zarr datasets.
