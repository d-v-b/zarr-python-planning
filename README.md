...
The user-facing APIs that don't fit into the other themes but that users have been asking for, in some cases for years. Five user-facing API areas: hierarchy navigation, chunk introspection, constructor and lifecycle UX, display and debugging, and IO conveniences.

→ [proposals/missing-apis.md](./proposals/missing-apis.md)

### Configuration, registries, and plugins

The runtime-configuration machinery — the untyped global `donfig` dict, the mutable global registries, and the entry-point plugin system — grew organically into three loosely-coupled mechanisms that users must operate *in combination* to do anything non-default. The v4 direction moves configuration from "global mutable state read implicitly" to "typed data passed explicitly" (the direction TensorStore, zarrs, and zarrita have each validated): a typed config object, array-scoped runtime config passed at open time, a registry redesign that addresses implementations by stable identity rather than Python qualnames and resolves plugin name-conflicts deliberately instead of picking one arbitrarily, and named profiles that replace global mutators like `enable_gpu()`. This is the substrate the performance-lever defaults (concurrency, caching, engine/preset selection) need a home for, so it lands first in the foundation tier.

→ [proposals/configuration.md](./proposals/configuration.md)
...
- **Configuration substrate replacement** — retire `donfig` for a typed config; redesign the registries for stable addressing and plugin-conflict resolution; move implementation selection into array-scoped config. The namespaced-key / env-override / preset surface survives. **This is a prerequisite, not a peer:** the dask-safe concurrency default, the cache default policy, preset selection, and engine selection all need a place to live, so the substrate must land *before* those perf-lever defaults — it is the first thing in this tier, resolving the earlier ambiguity over when it lands in its favor. → [proposals/configuration.md](./proposals/configuration.md)
