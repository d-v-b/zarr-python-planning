...
- **Migration sequencing.** Which core pieces extract first, and how long deprecation windows last, remain to be addressed in the implementation plan. The `Array`/`AsyncArray` sub-question has **resolved toward a single `Array` class that holds all the code and exposes async *selectively* via `*_async` methods on the IO-bound operations only — no public `AsyncArray`.** This keeps fast in-memory operations pure-sync (no event-loop overhead) and reserves async for operations where IO concurrency pays for itself. See [zarr-developers/zarr-python#4049](https://github.com/zarr-developers/zarr-python/pull/4049) and its discussion. The remaining open part is per-operation: for each `*_async` op, whether the sync side holds the logic (sync-first, addressing the async-task-overhead concern raised by `mkitti` in that thread) or the async side does (async-first, which genuinely suits high-latency cloud IO).
