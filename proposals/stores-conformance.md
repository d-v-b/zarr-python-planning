...
- **`CachingSpec`**. Cold read fetches from inner; warm read does not (asserted via a counting inner-store mock); writes invalidate the cache; eviction by `max_bytes` / `max_entries` / `ttl` each have a dedicated test. The full algorithm, defaults, write-invalidation contract, negative-caching semantics, and the full list of tests this spec runs are specified in [stores-caching.md](./stores-caching.md), including the recommended composition order with `RangeCoalescing` (caching outermost, coalescing below) and the migration plan from `experimental.cache_store`. `Caching[S]` is **key-agnostic**; tier-aware caching is a hierarchy-layer concern and is tested separately. The spec parameterizes over the sync/async variant — `{Caching, CachingAsync}` — so the same assertions run against both, with dispatch differing only in `await` vs direct call. This is the same pattern used by `SyncToAsyncSpec` / `AsyncToSyncSpec` below.
