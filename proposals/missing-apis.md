...
- **In-library rechunking primitive** — related to [rechunker](https://github.com/pangeo-data/rechunker) as prior art; the v4 ambition is a rechunking function that lives in `zarr-python` itself rather than a separate package, integrating with the engine boundary so high-performance engines can accelerate it.
